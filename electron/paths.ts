...
//
// 子目录：
//   - <dataRoot>/AnkeCreatorData/   → JSON 数据库
...
/**
 * JSON 数据库目录：<dataRoot>/AnkeCreatorData/
 * （自动创建目录 + 写入权限预检）
 */
export function getDataDir(): string {
  const dir = path.join(getDataRoot(), 'AnkeCreatorData')
 ...
 * AnkeCreatorData + images 复制到 <安装路径>/data\ 下。
 *
 * 触发条件（必须同时满足）：
 *   1. app.isPackaged（仅打包模式迁移，dev 模式不迁移）
 *   2. 新位置 <installPath>/data/ 下 AnkeCreatorData 目录不存在
 *   3. 旧位置 %APPDATA%\com.shanshian.ankecreator\AnkeCreatorData 存在
 *
 * 行为：
 *   - 用 fs.cpSync（递归）复制整个 AnkeCreatorData + images 到新位置
...
export function migrateFromUserDataIfNeeded(): void {
  ...
  const newDataDir = path.join(newRoot, 'AnkeCreatorData')
  ...
  try {
    if (
      ...
      console.log('[paths] 新位置已有 JSON 数据，跳过迁移:', newDataDir)
      return
    }
  } catch {
    /* 目录读取失败则继续尝试迁移 */
  ...
  const oldDataDir = path.join(oldRoot, 'com.shanshian.ankecreator', 'AnkeCreatorData')
  const oldImagesDir = path.join(oldRoot, 'com.shanshian.ankecreator', 'images')

  if (!fs.existsSync(oldDataDir)) {
    console.log('[paths] 旧位置无数据，无需迁移:', oldDataDir)
    return
  }
