...
//
// 数据存储位置（由 ./paths 决定）：
//  - 打包模式：<安装路径>/data/AnkeCreatorData/
//  - dev 模式：app.getPath('userData')/AnkeCreatorData/
// 首次启动打包版时会自动从 %APPDATA% 迁移到 <安装路径>/data/
//
// 文件结构（AnkeCreatorData/ 下）：
...
import { getDataDir as getDataDirFromPaths, migrateFromUserDataIfNeeded } from './paths';
...
/**
 * 数据目录包装：本地缓存 dataDir（避免每次重新计算路径）
 * 实际路径由 ./paths.getDataDir() 决定：
 *   - 打包模式：<安装路径>/data/AnkeCreatorData/
 *   - dev 模式：app.getPath('userData')/AnkeCreatorData/
 */
...
export function initMainDatabase(): void {
  ...
  // 打包模式首次启动时，自动把 %APPDATA% 下的旧数据迁移到 <安装路径>/data/
  // dev 模式下该函数直接 return，不影响开发
...
// ============================================================
// 收藏夹（favorites）—— 用户自建的故事分组容器
// 设计：每个收藏夹是用户给一组作品打的标签，类似 macOS Finder 智能文件夹
// 数据：
//   - favorites.json: Record<favoriteId, Favorite> 收藏夹元数据
//   - story_favorites.json: Array<{ story_id, favorite_id, added_at }> 关联表
// 业务规则：
//   - 删除收藏夹前必须先移出所有作品（仅空时删除）
//   - 故事被软删除/彻底删除时，自动从所有收藏夹中清除关联
// ============================================================
...
/** 列出所有收藏夹（按 created_at 升序） */
export function listFavorites(): FavoriteRow[] {
  const all = readTable<FavoriteRow>('favorites');
  return Object.values(all).sort(
...
export function createFavorite(data: { name: string }): FavoriteRow {
  ...
  return createRow<FavoriteRow>('favorites', {
    name: data.name.trim(),
  } as Omit<FavoriteRow, 'id' | 'created_at' | 'updated_at'>);
}
...
export function renameFavorite(id: string, name: string): FavoriteRow | null {
  ...
  return updateRow<FavoriteRow>('favorites', id, { name: name.trim() });
}

/** 统计收藏夹中的作品数 */
export function getFavoriteStoryCount(favoriteId: string): number {
  const assocs = readTable<StoryFavoriteRow>('story_favorites');
  let count = 0;
...
export function deleteFavoriteIfEmpty(id: string): { ok: boolean; error?: string } {
  ...
  const deleted = deleteRow<FavoriteRow>('favorites', id);
  if (!deleted) return { ok: false, error: '收藏夹不存在' };
  return { ok: true };
}

/** 把作品加入收藏夹（已存在则跳过） */
export function addStoryToFavorite(storyId: string, favoriteId: string): boolean {
  const all = readTable<StoryFavoriteRow>('story_favorites');
  ...
  // 收藏夹是否存在校验
  const favs = readTable<FavoriteRow>('favorites');
  if (!favs[favoriteId]) return false;
  // 故事是否存在校验
  const stories = readTable<StoryRow>('stories');
  if (!stories[storyId]) return false;
  ...
  all[newId] = row;
  writeTable('story_favorites', all);
  return true;
}
...
export function removeStoryFromFavorite(storyId: string, favoriteId: string): boolean {
  ...
  if (changed) writeTable('story_favorites', all);
  return changed;
}
