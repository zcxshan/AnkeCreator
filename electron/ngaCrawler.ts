...
interface TaskState {
  cancelled: boolean
  paused: boolean
  /** 用户对失败的处理决策：'continue' | 'skip' | undefined */
  userDecision?: 'continue' | 'skip'
}
...
interface ProgressPayload {
  ...
  message: string
  itemsFound?: number
  /** 抓取失败的页码列表（透传给 UI 用于决策弹窗） */
  failedPages?: number[]
  /** 标记当前是"等用户决策"暂停（连续失败 / 大量失败时） */
      ...
      try {
        ...
        for (let page = startPage; page <= maxPage; page++) {
          ...
          if (!pageFailed) {
            ...
            // 失败页：独立计数，连续 10 次自动暂停并通知用户决定（不与空页共享）
            failedPages.push(page)
            consecutiveFailedPages++
            if (!isRetryMode && consecutiveFailedPages >= 10) {
              console.log(
                `[nga:collect] 连续 ${consecutiveFailedPages} 页失败，暂停抓取等待用户决定（taskId=${taskId}）`,
              )
              // 自动暂停 + 通知用户决定
              state.paused = true
              ...
              send({
                ...
                total: totalProgress,
                phase: 'paused',
                message: `连续 ${consecutiveFailedPages} 页抓取失败。已自动暂停，请选择：继续尝试 / 停止抓取 / 跳过失败页并保存当前结果。`,
                itemsFound: allPosts.length,
                failedPages: [...failedPages],
                needsUserDecision: true,
              })
              // 等待用户决策：continue → 恢复抓取；skip → break；cancelled → break
              // 这里一直循环到 state.paused = false（用户选 continue/skip）或 state.cancelled = true
              while (state.paused && !state.cancelled) {
                await sleep(500)
              }
              if (state.cancelled) {
                // 用户选"停止抓取"
                console.log(`[nga:collect] 用户选择停止抓取（taskId=${taskId}）`)
                break
              }
              if (state.userDecision === 'skip') {
                // 用户选"跳过失败页并保存"
                console.log(`[nga:collect] 用户选择跳过失败页（taskId=${taskId}）`)
                ...
                // authorid 模式没法枚举剩余页（maxPage=Infinity），靠用户用"重试失败页"恢复已知失败页
                break
              }
              // state.userDecision === 'continue'：重置失败计数，继续抓取
              console.log(`[nga:collect] 用户选择继续抓取，重置失败计数（taskId=${taskId}）`)
              consecutiveFailedPages = 0
              // 继续外层 for 循环
            }
          }
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (decision === 'stop') {
        t.cancelled = true
        t.paused = false
        console.log(`[nga:collect:decide] 任务 ${taskId} 用户选择停止`)
      } else if (decision === 'continue') {
        t.paused = false
        t.userDecision = 'continue'
        console.log(`[nga:collect:decide] 任务 ${taskId} 用户选择继续`)
      } else if (decision === 'skip') {
        t.paused = false
        t.userDecision = 'skip'
        console.log(`[nga:collect:decide] 任务 ${taskId} 用户选择跳过失败页`)
      }
      return { ok: true }
    },
  )
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        // 真实末楼号：fetch 末页解析最后一楼 floor
        // authorid 模式末页 URL 也要带 authorid，否则会跳到全帖末页，结果不准
        let totalFloors = 0
        try {
          const lastPageUrl = authorid
            ? `${baseUrl}/read.php?tid=${tid}&authorid=${authorid}&page=${totalPages}`
            : `${baseUrl}/read.php?tid=${tid}&page=${totalPages}`
          ...
          if (lastResp.ok) {
            ...
            const lastCharset = detectCharsetFromHtml(lastBuffer)
            const lastHtml = new TextDecoder(lastCharset).decode(lastBuffer)
            const lastMax = extractMaxFloorFromHtml(lastHtml)
            if (lastMax > 0) {
              totalFloors = lastMax
            } else {
              // 兜底：估算值
              totalFloors = totalPages * 20
            }
          } else {
            totalFloors = totalPages * 20
          }
          ...
          console.warn(
            `[nga:fetchThreadInfo] 末页请求失败: ${(e as Error).message}`,
          )
          totalFloors = totalPages * 20
        }
        console.log(
          `[nga:fetchThreadInfo] tid=${tid}${authorid ? ` authorid=${authorid}` : ''} 总页数=${totalPages} 真实末楼=${totalFloors} charset=${charset}`,
        )
