...
export const platformName: 'electron' | 'capacitor' | 'web' = isElectron
  ? 'electron'
  : isCapacitor
  ? 'capacitor'
  : 'web';
 ...
 * Capacitor 平台标识
 * - 'android' | 'ios' | 'web'
 * - 仅在 isCapacitor 为 true 时有意义
 */
export const capacitorPlatform: 'android' | 'ios' | 'web' = isCapacitor
  ? ((window.Capacitor?.getPlatform?.() as 'android' | 'ios') ?? 'android')
  : 'web';
