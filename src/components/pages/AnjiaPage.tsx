...
export function AnjiaPage({ onBack }: AnjiaPageProps) {
  ...
  const [decision, setDecision] = useState<{
    ...
    message: string;
    failedPages: number[];
  }>({ open: false, taskId: 0, message: '', failedPages: [] });
  ...
  return (
    ...
    >
      ...
      <CollectDecisionDialog
        ...
        onContinue={async () => {
          if (decision.taskId) {
            await window.electronAPI?.decideNgaCollect?.(decision.taskId, 'continue');
          }
          setDecision((d) => ({ ...d, open: false }));
        }}
        onStop={async () => {
          if (decision.taskId) {
            await window.electronAPI?.decideNgaCollect?.(decision.taskId, 'stop');
          }
          setDecision((d) => ({ ...d, open: false }));
        }}
        onSkip={async () => {
          if (decision.taskId) {
            await window.electronAPI?.decideNgaCollect?.(decision.taskId, 'skip');
          }
          setDecision((d) => ({ ...d, open: false }));
        }}
      />
...
export function AnkeTab() {
  ...
  /** 收集安科：失败页号列表（用于重试 UI） */
  const [ankeFailedPages, setAnkeFailedPages] = useState<number[]>([]);
  /** 收集安科：已抓到的全部 items（重试时用于 floor 去重补齐） */
  const [ankeCollectedItems, setAnkeCollectedItems] = useState<unknown[]>([]);
  /** 收集安科：失败页列表展开状态 */
  const [ankeFailedPagesExpanded, setAnkeFailedPagesExpanded] = useState(false);
  /** 收集安科：重试中标记 */
  ...
  const [ankeCurrentTaskId, setAnkeCurrentTaskId] = useState(0);

  // 订阅实时进度事件
  useEffect(() => {
    if (!running && !ankeRetrying) return;
    if (!window.electronAPI?.onNgaCollectProgress) return;
    const unsub = window.electronAPI.onNgaCollectProgress((p) => {
      setProgress({ current: p.current, total: p.total });
      setProgressMsg(p.message);
      if (p.taskId) setAnkeCurrentTaskId(p.taskId);
      if (p.phase === 'paused') {
        setPaused(true);
        if (p.needsUserDecision && p.failedPages) {
          setAnkeDecision({
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      // 记录已抓 items + 失败页号（重试用）
      if (result.items) {
        setAnkeCollectedItems(result.items);
      }
      if (result.failedPages && result.failedPages.length > 0) {
        setAnkeFailedPages(result.failedPages);
      }
      ...
      if (result.actualMaxFloor && endF > result.actualMaxFloor) {
        if (result.failedPages && result.failedPages.length > 0) {
          warns.push(`⚠ 已爬到 ${result.actualMaxFloor} 楼，但还有 ${result.failedPages.length} 页抓取失败未补齐，实际帖子可能更长`);
        } else {
  ...
  /**
   * 重试收集安科失败页：复用已抓到的 items，仅重抓 failedPages，结果按 floor 去重补齐
   */
  const handleRetryFailedAnkePages = async () => {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        setAnkeFailedPages(ankeFailedPages); // 恢复
        return;
      }
      // 合并后的 items
      if (result.items) {
      ...
      if (newFailed.length > 0) {
        showToast(
          `已补齐 ${result.stats!.totalFloors} 楼，仍有 ${newFailed.length} 页未成功`,
          'warning',
        );
      } else {
      ...
      if (window.electronAPI?.saveStoryAsFile) {
        ...
        if (!res.canceled && res.fileName) {
          setResultMsg(
            `已重试并保存 ${result.stats!.totalFloors} 楼（${result.stats!.sectionCount} 节）：${res.fileName}\n请到「我的作品 → 📥 导入作品」选此 JSON 还原。`,
          );
        }
      }
    } catch (e) {
    ...
    <div className="flex-1 overflow-y-auto px-3 py-3 md:px-6 md:py-6">
      <div className="max-w-4xl mx-auto space-y-4 md:space-y-5">
        ...
        <CollectDecisionDialog
          ...
          onContinue={async () => {
            if (ankeDecision.taskId) {
              await window.electronAPI?.decideNgaCollect?.(ankeDecision.taskId, 'continue');
            }
            setAnkeDecision((d) => ({ ...d, open: false }));
          }}
          onStop={async () => {
            if (ankeDecision.taskId) {
              await window.electronAPI?.decideNgaCollect?.(ankeDecision.taskId, 'stop');
            }
            setAnkeDecision((d) => ({ ...d, open: false }));
          }}
          onSkip={async () => {
            if (ankeDecision.taskId) {
              await window.electronAPI?.decideNgaCollect?.(ankeDecision.taskId, 'skip');
            }
            setAnkeDecision((d) => ({ ...d, open: false }));
          }}
        />
