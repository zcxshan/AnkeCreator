...
const TUTORIAL_TOPICS: TutorialTopic[] = [
  ...
  },
  {
    ...
    icon: '📜',
    title: '骰子历史面板',
    summary: '查看、搜索、分组、删除骰子记录',
    steps: [
      { title: '1. 打开骰子历史面板', description: '在编辑页面右侧面板切换到「🎲骰点」tab，下方即为骰子历史记录区。所有在编辑器中投掷的骰子都会自动记录到此。' },
      { title: '2. 按作品绑定', description: '默认情况下，面板仅显示当前打开作品的骰子记录。每条记录都与具体的作品和章节小节绑定，作品导入导出时骰子历史会一并携带。' },
      { title: '3. 搜索筛选', description: '顶部搜索框支持按骰子名称、类型、结果、所属小节标题进行大小写不敏感搜索，实时过滤列表。' },
      { title: '4. 分组模式', description: '点击分组按钮可切换三种模式：「平铺」按时间倒序展示当前作品全部记录；「按时间」分为今天/昨天/本周/更早四档；「按作品」跨作品分组，可折叠展开每个作品。' },
      { title: '5. 单条删除与定位', description: '每条记录右侧有删除按钮，可单条移除。点击记录可跳转到对应骰子卡片所在小节，或恢复骰子到原位/插入未投掷的骰子。' },
    ],
  },
  {
    id: 'quick-dice-panel',
    icon: '⚡',
    title: 'QuickDicePanel 表达式骰子',
    summary: '在属性面板顶部快速投表达式骰子',
    steps: [
      { title: '1. 入口位置', description: '在编辑页面右侧面板切换到「⚙️属性」tab，顶部即为 QuickDicePanel 表达式骰子区域。' },
      { title: '2. 输入表达式', description: '在输入框中输入骰子表达式，如 <code>1d100</code>、<code>2d50+10</code>、<code>3d6</code> 等。支持四则运算和多个骰子组合。' },
      { title: '3. 投掷与动画', description: '点击「投骰」按钮或按回车键，骰子会播放与编辑器投骰相同的旋转动画和音效，结果显示在下方。' },
      { title: '4. 复制结果', description: '点击结果区域的「复制」按钮，可复制 NGA 风格的简单骰点文本，格式为 <code>[表达式=结果]</code>（如 <code>[1d100=42]</code>），方便粘贴到论坛或笔记中。' },
      { title: '5. 与编辑器骰子的区别', description: 'QuickDicePanel 仅用于快速投骰和复制结果，不会插入骰子卡片到正文，也不会记录到骰子历史。如需插入可交互骰子卡片，请使用工具栏的「🎲 骰子」按钮。' },
    ],
  },
  {
    id: 'dice-playground',
    icon: '🎲',
    title: 'DicePlayground 玩骰子',
    summary: '独立的玩骰子界面，带动画和音效',
    steps: [
      { title: '1. 进入玩骰子界面', description: '在首页或编辑器中找到「🎲 玩骰子」入口，进入独立的 DicePlayground 界面。' },
      { title: '2. 配置骰子', description: '选择骰子类型（数值骰子或选项骰子），设置面数、数量、修饰符或选项内容。' },
      { title: '3. 投掷动画', description: '点击投骰按钮，骰子会播放旋转动画和音效，与编辑器内投骰子完全一致的视听体验。' },
      { title: '4. 查看结果', description: '投掷完成后，结果显示区会展示总点数和详细分解。可以反复投掷，每次结果独立显示。' },
      { title: '5. 使用场景', description: '适合在写作时临时掷骰做决策（如角色命运判定、随机事件触发），不影响正文内容，也不会记录到骰子历史。如需记录到历史，请使用编辑器内的骰子卡片。' },
    ],
  },
  {
    id: 'export-epub',
